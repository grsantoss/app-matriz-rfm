...
document.addEventListener('DOMContentLoaded', function() {
    ...
    analyzeBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            const response = await fetch('/api/analyze-rfm', {
                method: 'POST',
                body: formData
            });
    ...
    async function updateAnalysisHistory() {
        try {
            ...
            if (!response.ok) {
                throw new Error('Erro ao obter histórico de análises');
            }
            ...
            historyEntries.forEach(entry => {
                ...
                dateTd.textContent = analysisDate.toLocaleDateString('pt-BR');
                tr.appendChild(dateTd);
                
                const sizeTd = document.createElement('td');
                sizeTd.textContent = `${entry.record_count} registros`;
                tr.appendChild(sizeTd);
                
                const actionTd = document.createElement('td');
                const downloadBtn = document.createElement('button');
                downloadBtn.className = 'btn btn-sm';
                downloadBtn.style.backgroundColor = '#ff014f';
